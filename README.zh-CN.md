...
自测覆盖文本变换、slugify、颜色检测、搜索、AES 往返、二维码生成、数据库往返、Windows 加密兼容辅助逻辑、Windows 导入器拒绝异常输入，以及图片粘贴路径行为。
...
选中多个文本剪贴项后，可以把它们合并为一个内容粘贴。分隔符可配置，顺序可反转，也可把合并结果另存为新剪贴项。
...
两台机器的网络密码必须一致。如果关闭接收，Ditto 仍可保留发送设置，但不会打开监听器。

## 导入与导出

### macOS 历史归档

Ditto 可以导出和导入本 macOS 移植版使用的自包含 SQLite 归档。
